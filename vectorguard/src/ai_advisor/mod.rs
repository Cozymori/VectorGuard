...
use crate::config::{AiAdvisorConfig, AiProvider, ApprovalMode};
use crate::event::{Action, EventType, NormalizedEvent};
use crate::fast_path::rules::RuleSet;
...
impl AiAdvisor {
    pub fn new(cfg: AiAdvisorConfig, rules_path: String) -> Self {
        let store = ApprovalStore::new(cfg.pending_dir.clone(), rules_path.clone());
        Self {
            cfg,
            store,
            rules_path,
    ...
    pub fn update_config(&mut self, cfg: AiAdvisorConfig) {
        self.store = ApprovalStore::new(cfg.pending_dir.clone(), self.rules_path.clone());
        self.cfg = cfg;
    }
    ...
    pub async fn analyze(&mut self, event: &NormalizedEvent) {
        ...
        let rs: RuleSet = match toml::from_str(&toml_block) {
            Ok(r)  => r,
            Err(e) => {
        ...
        let max = self.cfg.max_rules_per_fire;
        let proposed: Vec<_> = rs.rules.into_iter().take(max).collect();
        if proposed.is_empty() {
            warn!("AI advisor: response contained no rules");
            return;
        }

        let mode_label = match self.cfg.approval_mode {
            ApprovalMode::Manual => "pending",
            ApprovalMode::Auto   => "auto-approved",
        };
        let mut staged = 0;
        for rule in &proposed {
            let res = match self.cfg.approval_mode {
                ApprovalMode::Manual => self.store.stage_pending(rule),
                ApprovalMode::Auto   => self.store.stage_auto(rule),
            };
            match res {
                Ok(_)  => staged += 1,
                Err(e) => warn!("AI advisor: failed to stage rule '{}': {}", rule.name, e),
            }
        }
        info!("AI advisor: staged {}/{} rules as {}", staged, proposed.len(), mode_label);
    }
    ...
    fn build_prompt(&self, context: &[NormalizedEvent]) -> String {
        ...
        format!(
            "You are a security rule advisor for VectorGuard, an eBPF-based security monitoring tool.\n\
             ...
             {events}\n\
             \n\
             Based on this pattern, suggest 1-{max} new security rules. \
             Each rule MUST include a `description` field with a single-sentence \
             rationale explaining why the rule is being proposed — a human reviewer \
             will read this before approving.\n\
             \n\
             ...
             description       = \"why this rule\"   # one-sentence rationale (required)\n\
            ...
            max = self.cfg.max_rules_per_fire,
        )
    }
